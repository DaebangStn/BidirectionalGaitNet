...
{
    mEye = Eigen::Vector3d(2.0, 1.0, 2.0);
    ...
    mTrackball.setRadius(0.4);
}

void MusclePersonalizerApp::updateCamera()
{
    Eigen::Matrix3d rotation = mTrackball.getRotationMatrix();
    Eigen::Vector3d rotatedEye = rotation * (mEye * mZoom);
    Eigen::Vector3d center = mTrans;

    glMatrixMode(GL_PROJECTION);
    glLoadIdentity();
    gluPerspective(mPersp, (double)mWidth / (double)mHeight, 0.01, 100.0);

    glMatrixMode(GL_MODELVIEW);
    glLoadIdentity();
    gluLookAt(
        center(0) + rotatedEye(0), center(1) + rotatedEye(1), center(2) + rotatedEye(2),
...
void MusclePersonalizerApp::drawMuscles()
{
    ...
    // TODO: Implement muscle rendering with optional color coding by contracture
}
...
void MusclePersonalizerApp::mouseMove(double x, double y)
{
    ...
    if (mRotate) {
        mTrackball.updateBall(dx, dy);
    } else if (mTranslate) {
        Eigen::Matrix3d rot = mTrackball.getRotationMatrix();
        Eigen::Vector3d tx = rot.col(0);
        Eigen::Vector3d ty = rot.col(1);
        mTrans += (tx * dx * 0.002 + ty * (-dy) * 0.002);
        // Trackball center is screen space (2D), not world space
    }
