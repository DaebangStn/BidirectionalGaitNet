#!/usr/bin/env python3
"""
Main training script for HDF5-based regression MLP.

Usage:
    python train_hdf5.py --config config.yaml --hdf5 path/to/rollout_data.h5
"""
...
from hdf5_dataset import HDF5DataModule
...
def main():
    parser = argparse.ArgumentParser(description='Train regression MLP on HDF5 dataset')
    ...
    # Resolve HDF5 path to actual file
    resolved_hdf5_path = resolve_data_path(args.hdf5)
    print(f"Using HDF5 file: {resolved_hdf5_path}")

    # Override HDF5 path if provided
    if 'data' not in config:
        config['data'] = {}
    config['data']['hdf5_path'] = resolved_hdf5_path
    ...
    data_module = HDF5DataModule(
        ...
        in_lbl=data_config.get('in_lbl', ['Phase', 'Stride']),
        out_lbl=data_config.get('out_lbl', ['velocity', 'cot_ma15']),
