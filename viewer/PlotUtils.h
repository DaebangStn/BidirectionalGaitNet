#ifndef PLOT_UTILS_H
#define PLOT_UTILS_H

#include <implot.h>
#include <vector>
#include <string>
#include <map>

namespace PlotUtils
{
    /**
     * @brief Plot multiple time-series data with optional statistics
     * @param graphData Map of key -> value vectors
     * @param graphTime Map of key -> time vectors
     * @param keys Keys to plot
     * @param y_axis Which Y axis to use (ImAxis_Y1, ImAxis_Y2, etc.)
     * @param postfix Label postfix
     * @param show_stat Show statistics in legend
     * @param color_ofs Color offset for cycling through colors
     * @param timeStep Time step for converting indices to time
     */
    void plotGraphData(
        const std::map<std::string, std::vector<double>>& graphData,
        const std::map<std::string, std::vector<double>>& graphTime,
        const std::vector<std::string>& keys,
        ImAxis y_axis = ImAxis_Y1,
        const std::string& postfix = "",
        bool show_stat = false,
        int color_ofs = 0,
        double timeStep = 1.0
    );

    /**
     * @brief Compute statistics for graph data within a time range
     * @param graphData Map of key -> value vectors
     * @param graphTime Map of key -> time vectors
     * @param keys Keys to compute stats for
     * @param xMin Minimum time
     * @param xMax Maximum time
     * @return Map of key -> {mean, std, min, max}
     */
    std::map<std::string, std::map<std::string, double>> statGraphData(
        const std::map<std::string, std::vector<double>>& graphData,
        const std::map<std::string, std::vector<double>>& graphTime,
        const std::vector<std::string>& keys,
        double xMin,
        double xMax
    );

    /**
     * @brief Plot marker error over time
     * @param graphData Map of key -> value vectors
     * @param graphTime Map of key -> time vectors
     * @param xMin X-axis minimum
     * @param plotHeight Plot height in pixels
     */
    void plotMarkerError(
        const std::map<std::string, std::vector<double>>& graphData,
        const std::map<std::string, std::vector<double>>& graphTime,
        double xMin,
        float plotHeight = 200.0f
    );

} // namespace PlotUtils

#endif // PLOT_UTILS_H
